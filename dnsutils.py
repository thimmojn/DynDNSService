...
import dns.inet, dns.name, dns.query, dns.rcode, dns.rdatatype, dns.resolver, dns.tsig, dns.update
import ipaddress
...
class DomainUpdate:
    ...
    def addIPAddress(self, ipv4or6Address: str):
        rdType = DNSUtils.rdataTypeByIPAddress(ipv4or6Address)
        currentAddress = self.resolver.resolve(self.domainName, rdType)
        if currentAddress.response.rcode() == dns.rcode.NXDOMAIN:
            # create record
            self._request.absent(self.hostName, rdType)
            self._request.add(self.hostName, 300, rdType, ipv4or6Address)
        elif currentAddress[0].address != ipv4or6Address:
            # update record
            self._request.present(self.hostName, rdType)
            self._request.replace(self.hostName, 300, rdType, ipv4or6Address)
...
class DNSUtils:
    def rdataTypeByIPAddress(address: str) -> dns.rdatatype.RdataType:
        addressFamily = dns.inet.af_for_address(address)
        if addressFamily == AddressFamily.AF_INET:
            return dns.rdatatype.A
        elif addressFamily == AddressFamily.AF_INET6:
            return dns.rdatatype.AAAA
        else:
            raise ValueError('unhandled address family: {}'.format(addressFamily))
